...
int main() {
    ...
    while (true) {
        ...
        if (turnoIA) {
            ...
            int mejorMovimiento = 3;  // Movimiento predeterminado para la IA
            tablero.hacerMovimiento(mejorMovimiento, IA);
        } else {
            cout << "¡Es tu turno!" << endl;  // Mensaje que indica el turno del jugador
            int columna;

            // Leer la columna del jugador desde la consola (usando cin)
            cout << "Selecciona una columna (1-7): ";
            cin >> columna;

            // Validar que el movimiento sea válido
            while (columna < 1 || columna > 7 || !tablero.esMovimientoValido(columna - 1)) {
                cout << "Columna inválida o llena. Intenta con otra columna (1-7): ";
                cin >> columna;
            }

            // Hacer el movimiento
            tablero.hacerMovimiento(columna - 1, JUGADOR);
        }
